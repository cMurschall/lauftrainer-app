...
function sessionMap(request: Request): Record<string, string> {
  try {
    return JSON.parse(request.headers.get('X-Connector-Sessions') || '{}') as Record<string, string>
  } catch {
    return {}
  }
}
...
export async function listStatus(request: Request, env: Env) {
  ...
  const polarHeaders = new Headers(request.headers)
  polarHeaders.set('X-Polar-Session', sessions.polar || '')
  ...
  return json(
    {
      connectors: connectorRegistry.map((item) => ({
        ...item,
        connected: item.id === 'polar' ? polarConnected : stravaConnected,
        active: true,
      })),
    },
    200,
  ...
  if (active.has('polar')) {
    ...
    try {
      ...
      results.push({
        ...
        ...(response.ok ? {} : { error: result.detail || 'Polar-Sync fehlgeschlagen.' }),
      })
    } catch (error) {
      results.push({
        connector: 'polar',
        workouts: [],
        error: error instanceof Error ? error.message : 'Polar-Sync fehlgeschlagen.',
      })
    }
  }
  if (active.has('strava')) {
    try {
      ...
      results.push({
        connector: 'strava',
        workouts: [],
        error: error instanceof Error ? error.message : 'Strava-Sync fehlgeschlagen.',
      })
    }
  }
