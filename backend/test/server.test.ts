...
describe('worker routing', () => {
  it('returns health and CORS headers', async () => {
    const response = await server.fetch(
      new Request('http://worker.test/health', { headers: { Origin: 'http://localhost:5173' } }),
      env as never,
    )
  ...
  it('handles preflight and unknown routes', async () => {
    assert.equal(
      (await server.fetch(new Request('http://worker.test/api/x', { method: 'OPTIONS' }), env as never)).status,
      204,
    )
