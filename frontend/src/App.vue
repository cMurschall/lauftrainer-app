...
import { defaultAppSettings, type AppSettings, type ThemePreference } from './types/settings'
import AppSidebar from './components/AppSidebar.vue'
import BackendStatus from './components/BackendStatus.vue'
import type { TrainingPlanDay, UserConfig, Workout } from './types/workout'
const route = useRoute(); const { locale, t, setLocale, formatTime } = useI18n()
const workouts = ref<Workout[]>([]), plan = ref<TrainingPlanDay[]>([]), message = ref(''), loading = ref(false), consent = ref(false), search = ref('')
const backendStatus = ref<'checking' | 'online' | 'offline'>('checking'), backendVersion = ref('–'), backendCheckedAt = ref(''); const polarConnected = ref(false), polarLoading = ref(false); const theme = ref<ThemePreference>(defaultAppSettings.theme)
...
const analysis = computed(() => calculateAnalysis(workouts.value, config.value)); useTheme(theme)
onMounted(async () => { workouts.value = await workoutDb.list(); const stored = await workoutDb.getConfig(); if (stored) config.value = stored; const saved = await workoutDb.getAppSettings(); if (saved) { theme.value = saved.theme; setLocale(saved.locale) } else { const old = localStorage.getItem('lauftrainer-locale'); if (old === 'de' || old === 'en') setLocale(old); await saveSettings() }; capturePolarSession(); await checkBackend(); polarConnected.value = await polarStatus().catch(() => false) })
async function saveSettings() { const settings: AppSettings = { theme: theme.value, locale: locale.value }; await workoutDb.saveAppSettings(settings) }
...
<template><div class="app-layout"><AppSidebar /><main class="main-content"><header class="topbar"><div><span class="mobile-brand">LaufTrainer</span><span class="route-label">{{ route.name === 'settings' ? t.settingsNav : t.dashboard }}</span></div><span class="badge">{{ t.offlineReady }}</span></header><RouterView v-slot="{ Component }"><component :is="Component" v-bind="route.name === 'settings' ? { config, theme, saveConfig, saveSettings, downloadBackup, restoreBackup, clearData } : { workouts, plan, config, analysis, search, message, loading, consent, polarConnected, polarLoading, importFiles, createPlan, connectPolar, syncPolar }" @update:search="search = $event" @update:consent="consent = $event" @update:theme="theme = $event" @update:locale="setLocale($event)" /></RouterView><footer><span>{{ t.localUpload }}</span><BackendStatus :status="backendStatus" :version="backendVersion" :checked-at="backendCheckedAt" :check="checkBackend" /></footer></main></div></template>
