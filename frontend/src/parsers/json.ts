...
export function parsePolarJson(text: string, fileName: string): Workout {
  ...
  return { id: `json-${String(raw.id || fileName)}-${date}`, source: 'polar-json', name: String(raw.name || fileName), sport: String(raw.sport || 'RUNNING'), date, durationSeconds: duration > 10000 ? duration : duration * 60, distanceKm: distance > 1000 ? distance / 1000 : distance, averageHeartRate: Number(raw.average_heart_rate || raw.averageHeartRate || raw.avg_hr) || undefined, calories: Number(raw.calories) || undefined, records: [], importedAt: new Date().toISOString() }
}
