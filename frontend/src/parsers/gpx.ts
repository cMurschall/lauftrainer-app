...
function distanceMeters(a: ActivityRecord, b: ActivityRecord): number {
    ...
    const start = radians(a.latitude), end = radians(b.latitude)
...
export function parseGpx(text: string, fileName: string): Workout {
    ...
    for (let index = 1; index < records.length; index += 1) {
        ...
        const previous = records[index - 1].altitudeM, current = records[index].altitudeM
    ...
    return {
        ...
        source: 'gpx', name: fileName, sport: 'RUNNING', date,
        durationSeconds: Math.max(0, durationSeconds), distanceKm: distanceM / 1000,
        ascentM, records, importedAt: new Date().toISOString()
    }
}
