...
export function calculateAnalysis(workouts: Workout[], config: UserConfig): AnalysisSummary {
  const weeks = new Map<string, { workoutCount: number; distanceKm: number; durationMinutes: number; load: number; heartRates: number[] }>()
  ...
  for (const workout of workouts) {
    ...
    current.workoutCount += 1; current.distanceKm += distanceKm; current.durationMinutes += durationMinutes
    ...
    totalDistanceKm += distanceKm; totalDurationMinutes += durationMinutes
  ...
  return { totalDistanceKm, totalDurationMinutes, zoneMinutes, weekly: [...weeks.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([weekStartValue, value]) => ({ weekStart: weekStartValue, workoutCount: value.workoutCount, distanceKm: value.distanceKm, durationMinutes: value.durationMinutes, trainingLoad: value.load, averageHeartRate: value.heartRates.length ? value.heartRates.reduce((a, b) => a + b, 0) / value.heartRates.length : undefined })) }
}
