...
function fosterFromDaily(load: Array<{ date: string; value: number }>): FosterWeek[] {
    return [...weekMap(load, item => isoWeekStart(item.date)).entries()].map(([weekStart, entries]) => {
...
export function calculateRpeFosterMetrics(workouts: Workout[]): FosterWeek[] {
    return fosterFromDaily(workouts.filter(w => finite(w.sessionRpe) && w.sessionRpe >= 1 && w.sessionRpe <= 10).map(w => ({
        date: toDateKey(w.date),
...
export function calculateHrZoneDistribution(workouts: Workout[], config: UserConfig): HrZoneWeek[] {
    ...
    return [...map.entries()].map(([weekStart, minutes]) => {
...
export function calculateEfficiency(workouts: Workout[]): EfficiencyPoint[] {
    return workouts.filter(w => normalizeSport(w.sport) === 'Running' && finite(w.distanceKm) && w.distanceKm > 0 && w.durationSeconds > 0 && finite(w.averageHeartRate) && w.averageHeartRate > 0).map(w => ({
        date: toDateKey(w.date),
        workoutId: w.id,
        efficiency: (w.distanceKm! / (w.durationSeconds / 3600)) / w.averageHeartRate!
    })).filter(x => x.date && Number.isFinite(x.efficiency)).sort((a, b) => a.date.localeCompare(b.date))
}
