...
export async function requestTrainingPlan(workouts: Workout[], config: UserConfig): Promise<TrainingPlanDay[]> {
  const response = await fetch(`${API_URL}/training-plan`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ config, workouts: workouts.slice(0, 14).map(workout => ({ date: workout.date, sport: workout.sport, duration_minutes: Math.round(workout.durationSeconds / 60), distance_km: workout.distanceKm, avg_hr: workout.averageHeartRate, calories: workout.calories })) }) })
