...
const props = defineProps<{ config: UserConfig; theme: ThemePreference; connectors: ConnectorSettings[]; saveConfig: () => void; saveSettings: () => void; downloadBackup: () => void; restoreBackup: (event: Event) => void; clearData: () => void; connectConnector: (id: 'polar') => void }>()
...
<template>
  ...
  <section class="card settings-section"><p class="eyebrow">{{ t.connectors }}</p><div class="connector-list"><article v-for="connector in props.connectors" :key="connector.id" class="connector-row"><div><strong>{{ connector.name }}</strong><span class="muted">{{ connector.connected ? t.connectorConnected : t.connectorNotConnected }}</span></div><div class="connector-controls"><button v-if="!connector.connected" class="button secondary" @click="connectConnector(connector.id)">{{ t.connectConnector }}</button><label v-else class="switch"><input v-model="connector.active" type="checkbox" @change="saveSettings"><span>{{ connector.active ? t.connectorActive : t.connectorInactive }}</span></label></div></article></div></section>
