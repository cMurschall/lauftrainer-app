...
<template>
  ...
  <section class="hero">
    <div><h2>{{ t.heroTitle }}</h2>
      <p>{{ t.heroText }}</p></div>
    <span class="hero-orb">◌</span></section>
  ...
  <section class="stats">
    ...
    <article class="card"><span>{{ t.totalDistance }}</span><strong
        class="metric">{{ analysis.totalDistanceKm.toFixed(1) }} km</strong></article>
    <article class="card"><span>{{ t.trainingTime }}</span><strong
        class="metric">{{ Math.round(analysis.totalDurationMinutes / 60) }} h</strong></article>
  </section>
  <section class="grid">
    <article class="card"><p class="eyebrow">{{ t.history }}</p><input :placeholder="t.search" :value="search"
                                                                       class="search"
                                                                       @input="emit('update:search', ($event.target as HTMLInputElement).value)">
      <ul class="workouts">
        <li v-for="workout in filteredWorkouts.slice(0, 12)" :key="workout.id"><strong>{{ workout.date }}</strong><span>{{
            workout.sport
    ...
    <article class="card"><p class="eyebrow">{{ t.weekOverview }}</p>
      <div class="weeks">
        <div v-for="week in analysis.weekly.slice(-8).reverse()" :key="week.weekStart"><strong>{{
            week.weekStart
          }}</strong><span>{{ week.distanceKm.toFixed(1) }} km · {{ week.workoutCount }} {{ t.units }}</span><i
            :style="{ width: `${Math.min(100, week.distanceKm * 3)}%` }"></i></div>
        <p v-if="!analysis.weekly.length">{{ t.noWeeks }}</p></div>
    </article>
  </section>
  <section class="card">
    <div class="card-heading">
      <div><p class="eyebrow">{{ t.connectors }}</p>
        <h3>{{ t.syncAllConnectors }}</h3></div>
      <span class="connection-dot"></span></div>
  ...
  <section class="card"><p class="eyebrow">{{ t.aiPlan }}</p>
    <p>{{ t.aiDescription }}</p><label class="consent"><input :checked="consent" type="checkbox"
    ...
    <div v-if="plan.length" class="plan">
      <div v-for="day in plan" :key="day.day"><strong>{{ day.day }}</strong><span>{{
          day.description
        }} · {{ day.total_duration_minutes }} min</span></div>
    </div>
  </section>
</template>
